...
* 当然，复制进程地址空间时，采用了cow技术，延迟复制的时间
    ...
    * 父进程调用fork时，会把它的地址空间设置为只读（修改页表PTE_W=0），子进程和父进程的页表完全相同，那么访问某虚拟地址会映射到同一个物理页帧。当其中任何一个进程修改此用户内存空间中的某页面时，ucore会通过page fault异常获知该操作，并复制此物理页帧，然后将复制后的页面分配给修改进程，而不会影响其他进程。

* cow引发的脏牛漏洞：
    * 设法写一个只读物理页，如果它拿到权限相关的物理页，就可能实现提权，从普通用户权限提升为超级用户权限。 



## sys_execve:

1. 释放原进程的几乎所有资源（分配的内存空间，页表），只保留原进程的pid，属性、原进程与其他进程之间的关系等等。
2. 读取可执行文件，并根据ELFheader分配特定位置的虚拟内存，并加载代码与数据至特定的内存地址，最后分配堆栈并设置trapframe属性，设置置新进程名称。
...
* 检查自己是否有子进程：
  如果自己此时还有子进程，那么将它们的父进程设置为1号init_proc，这样倘若这些子进程进入结束状态，则1号进程可以代为回收资源。

* 检查父进程是否阻塞在wait/waitpid

    * 如果父进程还未阻塞在wait/waitpid，则该进程进入zombie状态，cpu调度其他进程执行

    * 如果父进程已经阻塞在wait/waitpid，则唤醒父进程，由父进程回收该进程的PCB和内核栈
...
当该进程的某个子进程结束运行后，当前进程会被唤醒，并在do_wait函数中回收子进程的PCB和内核栈
...
假定有两个用户进程a和b，在二者进行进程切换的过程中，具体的步骤如下：
...
在时钟中断前会保存当前进程a的trapframe, 并检查need_resched标记。如果标记为1，说明当前进程已经用完了时间片，需要切一个换新进程。
操作系统会调用schedule函数，根据进程调度算法选择一个新的进程，在switch_to中切换上下文：
...
切换结束后，从此时的context->eip开始执行，那么就回到了b的用户空间；

在执行b进程的过程中，如果在某次时钟中断后，发现b的时间片为0，那么会再次调用schedule(), 假如选择进程a执行，在switch_to中切换为a的上下文。那么此时的context->eip正是a进程在内核中执行schedule()后的下一条指令，那么iret指令会弹出trapframe，从而回到进程a的用户空间接着执行。
